...
class Rig(rclpy.node.Node):
    ...
    def drive_stamped_cb(self, drive_stamped: AckermannDriveStamped):
        ...
        self.throttle.target_duty = 90 if abs(drive_stamped.drive.acceleration) > 0 else 0
        if drive_stamped.drive.acceleration > 0:
            self.direction = "forwards"
        elif drive_stamped.drive.acceleration < 0:
